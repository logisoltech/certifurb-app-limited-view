...
const AddAuctionProductPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Create product specs array from the form fields
      const productSpecs = [];

      // Add CPU if available
      if (cpu.trim()) {
        productSpecs.push(cpu.trim());
      }

      // Add RAM if available
      if (ram) {
        productSpecs.push(ram);
      }

      // Add storage if available
      if (getFieldCount() > 2 && storage) {
        productSpecs.push(storage);
      }
      ...
      if (auctionTimer.trim()) {
        ...
        console.log("UTC Conversion Debug:", {
          originalInput: auctionTimer,
          localDate: localDate.toISOString(),
          localDateLocal: localDate.toLocaleString(),
          timezoneOffset: localDate.getTimezoneOffset(),
          utcDate: utcDate.toISOString(),
          utcDateLocal: utcDate.toLocaleString(),
          formattedForBackend: formattedUTC,
      ...
      const auctionProductData = {
        ...
        auction_timer: formattedUTC, // Always store UTC
      };
      ...
      if (result.success) {
        ...
        setAudio("");
        setBrand("");
        setAuctionTimer(""); // Reset auction timer
      } else {
