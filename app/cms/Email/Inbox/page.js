...
const InboxPage = () => {
  ...
  const fetchEmails = async (folder = 'inbox', page = 1, search = '') => {
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/cms/emails?${queryParams}`);
  ...
  const fetchEmailStats = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/cms/emails/stats');
      const data = await response.json();
      if (data.success) {
  ...
  // Manual email fetch from IMAP
  const handleFetchEmails = async () => {
    setIsFetchingEmails(true);
    try {
      const response = await fetch('http://localhost:3001/api/cms/emails/fetch', {
        method: 'POST',
        headers: {
  ...
  const toggleStar = async (emailId, currentlyStarred) => {
    try {
      const response = await fetch(`http://localhost:3001/api/cms/emails/${emailId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleBulkAction = async (action) => {
    ...
    try {
      const response = await fetch('http://localhost:3001/api/cms/emails/bulk', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
                ...
                <button 
                  ...
                  className={`flex items-center space-x-1 px-3 py-2 rounded-lg ${
                    isFetchingEmails 
                      ...
                      : 'hover:bg-green-100 hover:text-green-600 bg-green-50 text-green-600'
                  }`}
                >
                  <ArrowPathIcon className={`h-4 w-4 ${isFetchingEmails ? 'animate-spin' : ''}`} />
                  <span>{isFetchingEmails ? 'Fetching...' : 'Fetch New Emails'}</span>
                </button>
                <button 
                  onClick={() => {
                    fetchEmails(currentFolder, pagination.page, searchTerm);
            ...
            {loading ? (
              ...
              <div className="flex items-center justify-center h-64">
                <div className="text-center">
                  <div className="text-gray-500 mb-2">No emails found</div>
                  <button 
                    onClick={handleFetchEmails}
                    className="text-blue-600 hover:text-blue-800"
                  >
                    Try fetching new emails
                  </button>
                </div>
              </div>
            ) : (
